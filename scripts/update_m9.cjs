...
m9.live_commentary = comms.map((c, index) => {
  ...
  else if (c.comment && c.comment.includes('(Arsenal)')) team = 'home';
  else if (c.comment && c.comment.includes('(Chelsea)')) team = 'away';
...
console.log('Successfully updated m9 in fixtures.json! Total commentary events:', m9.live_commentary.length);
